...
# Creator: Zahid Islam
# Fixed Version: Added Renew Feature + Payment Proof Fix
...
install_dependencies() {
    ...
    apt-get install -y python3 python3-pip python3-venv git wget curl openssl ufw
}
...
install_udp_vpn() {
    ...
    mkdir -p /etc/zivpn
    wget https://raw.githubusercontent.com/zahidbd2/udp-zivpn/main/config.json -O /etc/zivpn/config.json -q
...
EOF
    ...
    # Create database.py with renew feature
...
class Database:
    ...
    def get_account_status(self, account_id):
        cursor = self.conn.cursor()
        cursor.execute('''
            SELECT is_active, expire_date FROM accounts WHERE id = ?
        ''', (account_id,))
        return cursor.fetchone()
EOF

    # Create bot.py with fixed payment proof and renew feature
...
main_installation() {
    ...
    if [[ "$install_bot_choice" =~ ^[Yy]$ ]]; then
        ...
        echo -e "3. Check bot token in: nano /opt/zivpn-bot/.env"
        echo -e "4. Restart bot: systemctl restart zivpn-bot.service"
        echo -e "5. Make sure you started the bot with @BotFather"
    fi
}
