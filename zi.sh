...
# Creator: Zahid Islam
# Fixed Version: Added Screenshot Display + Keyboard Menu
...
EOF
    ...
    if systemctl is-active --quiet zivpn.service; then
        ...
        journalctl -u zivpn.service -n 10 --no-pager | grep -i "auth\|password\|error\|fail"
    else
    ...
    if [ -f /etc/zivpn/config.json ]; then
        ...
        python3 -m json.tool /etc/zivpn/config.json 2>/dev/null || cat /etc/zivpn/config.json
        ...
        # Test connection with curl (simple test)
        print_status "Testing VPN port..."
        if nc -z -u localhost 5667 2>/dev/null; then
...
EOF
    ...
    # Create database.py with improved payment system and VPN config update
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    print(f"DEBUG: VPN service restarted successfully")
                    return True
                else:
                    print(f"DEBUG: Failed to restart VPN service: {result.stderr}")
                    return False
            except subprocess.TimeoutExpired:
                print(f"DEBUG: VPN service restart timeout")
                return False
            except Exception as e:
                print(f"DEBUG: Error restarting VPN: {e}")
                return False
                
        except FileNotFoundError:
            print(f"DEBUG: Config file not found at {Config.VPN_CONFIG_PATH}")
            return False
        except json.JSONDecodeError as e:
            print(f"DEBUG: Invalid JSON in config: {e}")
            return False
        except Exception as e:
            print(f"DEBUG: Error updating VPN config: {e}")
            return False
...
EOF
    ...
    # Create bot.py with Keyboard Menu and fixed payment system
...
"""
    ...
    async def receive_payment_proof(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Receive payment proof from user"""
        user = update.effective_user
        user_id = user.id
        ...
        # Notify all admins with the screenshot and inline buttons
...
fix_vpn_config() {
    ...
    if [ -f /etc/zivpn/config.json ]; then
        ...
        if python3 -m json.tool /etc/zivpn/config.json > /dev/null 2>&1; then
            ...
            # Show current passwords in config
...
try:
    ...
    if 'auth' in config and 'config' in config['auth']:
        print('Passwords:', config['auth']['config'])
    else:
...
"
            ...
            print_error "VPN config.json is invalid JSON"
            
            # Try to fix it
            print_status "Attempting to fix config.json..."
...
EOF
            ...
            print_success "Fixed config.json with passwords: ${VPN_PASSWORDS[*]}"
        fi
    else
        print_error "VPN config.json not found"
...
EOF
        ...
        print_success "Created config.json with passwords: ${VPN_PASSWORDS[*]}"
    fi
...
main_installation() {
    ...
    if [[ "$install_bot_choice" =~ ^[Yy]$ ]]; then
        ...
        echo -e "3. Check VPN logs: journalctl -u zivpn.service -f"
        echo -e "4. Test VPN connection:"
        echo -e "   Server: ${SERVER_HOSTNAME:-$SERVER_IP}:5667"
        echo -e "   Password: ${VPN_PASSWORDS[0]}"
        echo -e "5. Restart services if needed:"
        echo -e "   systemctl restart zivpn.service"
        echo -e "   systemctl restart zivpn-bot.service"
    fi
}
