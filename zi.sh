#!/usr/bin/env bash
# install_zivpn_bot_full_fixed.sh
# Full installer: ZIVPN server + Telegram bot (fixed: reliable config updates + service restart)
# Usage: sudo bash install_zivpn_bot_full_fixed.sh
set -euo pipefail
...
if command -v curl >/dev/null 2>&1; then
  curl -fsSL "$ZIVPN_RELEASE_URL" -o "$ZIVPN_BIN" || true
elif command -v wget >/dev/null 2>&1; then
  wget -qO "$ZIVPN_BIN" "$ZIVPN_RELEASE_URL" || true
else
  echo "⚠️ Please install curl or wget and re-run this installer."
fi
chmod +x "$ZIVPN_BIN" || true

# Ensure config.json exists and has listen + config array
...
JSON
  ...
  # ensure listen field exists or set it
  if grep -q '"listen"' "$ZIVPN_ETC/config.json"; then
    ...
    if command -v jq >/dev/null 2>&1; then
      tmpf=$(mktemp)
      jq '. + {"listen":"0.0.0.0:5667"}' "$ZIVPN_ETC/config.json" > "$tmpf" && mv "$tmpf" "$ZIVPN_ETC/config.json" || true
    else
      # naive insert after opening brace
      awk 'NR==1{print; print "  \"listen\": \"0.0.0.0:5667\","; next} {print}' "$ZIVPN_ETC/config.json" > "$ZIVPN_ETC/config.json.tmp" && mv "$ZIVPN_ETC/config.json.tmp" "$ZIVPN_ETC/config.json" || true
    fi
  fi
fi
...
sysctl -w net.ipv4.ip_forward=1 1>/dev/null 2>/dev/null || true
...
# network detection
INET_IF=$(ip -4 route ls | awk '/default/ {for(i=1;i<=NF;i++) if($i=="dev") print $(i+1); exit}' || echo "eth0")
LOCAL_IP=$(ip -4 addr show dev "$INET_IF" scope global | awk '/inet /{print $2}' | cut -d/ -f1 | head -1 || true)
if [ -z "$LOCAL_IP" ]; then
  LOCAL_IP=$(hostname -I 2>/dev/null | awk '{print $1}' || true)
fi
if [ -z "$LOCAL_IP" ]; then
  echo "⚠️ Could not auto-detect local IP. Using 127.0.0.1 for DNAT placeholder."
  LOCAL_IP="127.0.0.1"
fi

# Configure iptables NAT correctly (remove old placeholder if exists then add)
iptables -t nat -D PREROUTING -i "$INET_IF" -p udp --dport 6000:19999 -j DNAT --to-destination ":5667" 1>/dev/null 2>/dev/null || true
iptables -t nat -D PREROUTING -i "$INET_IF" -p udp --dport 6000:19999 -j DNAT --to-destination "$LOCAL_IP":5667 1>/dev/null 2>/dev/null || true
iptables -t nat -A PREROUTING -i "$INET_IF" -p udp --dport 6000:19999 -j DNAT --to-destination "$LOCAL_IP":5667
iptables -t nat -A POSTROUTING -o "$INET_IF" -j MASQUERADE 1>/dev/null 2>/dev/null || true

# ensure accept rules for those udp ports
iptables -I INPUT -p udp --dport 5667 -j ACCEPT 1>/dev/null 2>/dev/null || true
iptables -I INPUT -p udp --dport 6000:19999 -j ACCEPT 1>/dev/null 2>/dev/null || true

# persist iptables if possible
if command -v netfilter-persistent >/dev/null 2>&1; then
  netfilter-persistent save 1>/dev/null 2>/dev/null || true
elif dpkg-query -W -f='${Status}' iptables-persistent 2>/dev/null | grep -q "install ok installed"; then
...
echo "Detected interface: $INET_IF, local IP: $LOCAL_IP"

# --- Bot config prompts ---
echo -e "\n--- Telegram Bot Setup ---"
read -rp "Enter Your Server Public IP (leave empty to use detected $LOCAL_IP): " INPUT_SERVER_IP
...
read -rp "Enter Hostname (e.g., jvpn.com) : " HOSTNAME
...
# --- Bot code (full) ---
# This bot: updates /etc/zivpn/config.json safely (JSON edit), and restarts zivpn.service after changes.
cat > "$BOT_PY" <<'PY'
#!/usr/bin/env python3
# bot.py - ZIVPN Telegram Bot (with safe config updates and service restart)
import os, json, time, datetime, uuid, socket, subprocess, logging
...
# try to auto-detect server ip if not set
if not SERVER_IP:
    try:
...
# Conversation states
(CREATE_USERNAME, CREATE_PASSWORD, CHOOSE_DURATION) = range(3)
...
async def create_duration_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Add password to zivpn config (safe JSON edit)
    try:
        zcfg = read_json(ZIVPN_CONFIG, {})
        if not isinstance(zcfg, dict):
            zcfg = {}
        cfg_list = zcfg.get("config", [])
        if not isinstance(cfg_list, list):
            cfg_list = [str(x) for x in cfg_list]
        if upass not in cfg_list:
            cfg_list.append(upass)
            zcfg["config"] = cfg_list
            write_json(ZIVPN_CONFIG, zcfg)
            # restart service so new password applies
...
async def user_account_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("user_delete_"):
        ...
        try:
            zcfg = read_json(ZIVPN_CONFIG, {})
            cfg_list = zcfg.get("config", [])
            if acc["password"] in cfg_list:
                cfg_list = [p for p in cfg_list if p != acc["password"]]
                zcfg["config"] = cfg_list
                write_json(ZIVPN_CONFIG, zcfg)
                restart_zivpn_service()
        except Exception as e:
...
async def user_renew_choose_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cost is None or credit < cost:
        await query.edit_message_text("❌ Invalid or insufficient credit.", reply_markup=main_menu_kb())
        return
    u["credit"] = credit - cost
    write_json(USERS, users)
    accounts = read_json(ACCOUNTS, [])
...
async def text_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt in ["🔁 Renew Account","renew account","Renew Account"]:
        return await my_accounts_cmd(update, context)
    if txt in ["🛠 Admin Panel","admin panel","Admin Panel"]:
...
def main():
    ...
    conv = ConversationHandler(
        entry_points=[CommandHandler("create", create_account_cmd), MessageHandler(filters.Regex(r"^(?i)(create account|👤 Create Account)$"), create_account_cmd)],
        states={
...
apt-get install -y python3-venv python3-pip curl jq iptables-persistent ufw -y 1>/dev/null 2>/dev/null || true
...
echo ""
echo "Checklist:"
echo "  - ZIVPN config: $ZIVPN_ETC/config.json"
echo "  - Bot config: $BOT_CONFIG"
echo "  - Users: $USERS_JSON"
echo "  - Accounts: $ACCOUNTS_JSON"
echo "  - Topups: $TOPUPS_JSON"
echo ""
echo "Run checks if clients can't connect:"
echo "  ss -ulpen | grep 5667"
echo "  iptables -t nat -L -n -v"
echo "  ufw status verbose"
echo "  journalctl -u zivpn.service -n 200 --no-pager"
echo "  journalctl -u zivpn-bot.service -n 200 --no-pager"
echo ""
echo "Done."
