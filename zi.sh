...
# Creator: Zahid Islam
# Fixed Issues: 
# 1. Admin payment notification with screenshot and buttons ✓ FIXED
# 2. Create account conversation flow ✓ FIXED  
# 3. Default password issue ✓ FIXED
...
echo -e "${BLUE}Fixed Version: 3.0${NC}"
echo ""
...
collect_configuration() {
    ...
    # Get VPN passwords - REMOVED DEFAULT 'zi' PASSWORD
    echo ""
    echo -e "${YELLOW}=== VPN Passwords ===${NC}"
    echo "Note: You can add passwords manually later via Admin Panel"
    read -p "Enter VPN passwords separated by commas (or press Enter to skip for now): " vpn_passwords_input
    ...
    if [ -n "$vpn_passwords_input" ]; then
        ...
        VPN_PASSWORDS=()
        echo "No default passwords set. Users will create their own passwords."
    fi
...
install_udp_vpn() {
    ...
    # Create JSON array from VPN passwords (empty array if no passwords)
    config_array="["
...
EOF
    ...
    if [ ${#VPN_PASSWORDS[@]} -gt 0 ]; then
        ...
        print_success "Config.json created with empty password list"
        print_status "Users will add passwords when creating accounts"
    fi
...
EOF
    ...
    if [ ${#VPN_PASSWORDS[@]} -gt 0 ]; then
        ...
        echo "VPN Passwords: No default passwords (users will create their own)"
    fi
...
class Database:
    ...
    def create_account(self, user_id, username, password, days):
        ...
        try:
            ...
            # Update VPN config - REMOVED DEFAULT 'zi' PASSWORD
            success = self.update_vpn_config(password)
            if not success:
                self.conn.rollback()
                return False, "Failed to update VPN configuration"
    ...
    def update_vpn_config(self, password):
        ...
        try:
            ...
            # Add password if not exists - FIXED: Only add the user's password
    ...
    def create_payment(self, user_id, amount, screenshot=None):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, 'pending')
        ''', (user_id, amount, screenshot, 'pending'))
        self.conn.commit()
...
EOF
    ...
    # Create bot.py with COMPLETELY FIXED payment notification and conversation flow
...
class ZivpnBot:
    ...
    def setup_handlers(self):
        ...
        # Keyboard button handlers - FIXED: Use filters.Text for regex
        self.application.add_handler(MessageHandler(filters.Text("💳 Top-up Credit"), self.topup_keyboard))
        self.application.add_handler(MessageHandler(filters.Text("🆕 Create Account"), self.create_account_keyboard))
        self.application.add_handler(MessageHandler(filters.Text("🔄 Renew Account"), self.renew_account_keyboard))
        self.application.add_handler(MessageHandler(filters.Text("💰 Check Credit"), self.check_credit_keyboard))
        self.application.add_handler(MessageHandler(filters.Text("👤 My Accounts"), self.my_accounts_keyboard))
        self.application.add_handler(MessageHandler(filters.Text("👑 Admin Panel"), self.admin_panel_keyboard))
        self.application.add_handler(MessageHandler(filters.Text("🏠 Main Menu"), self.back_to_menu_keyboard))
        ...
        # Conversation handlers with proper entry points
        topup_conv = ConversationHandler(
            ...
            states={
                ...
                PAYMENT_PROOF: [MessageHandler(filters.PHOTO | filters.TEXT & ~filters.COMMAND, self.receive_payment_proof)]
            },
        ...
        # Default message handler - FIXED: Only handle unknown messages
        self.application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & ~filters.Regex('^💳|🆕|🔄|💰|👤|👑|🏠'), self.handle_unknown_message))
...
"""
    ...
    async def handle_unknown_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if context.user_data.get('expecting_password') or context.user_data.get('expecting_username'):
            # We're in the middle of account creation, let the conversation handler handle it
            return
...
"""
    ...
    async def upload_payment_proof(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await query.edit_message_text("📸 Please send the payment screenshot now:")
        return PAYMENT_PROOF
    
    async def receive_payment_proof(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Receive payment proof from user - COMPLETELY FIXED VERSION"""
        try:
            user = update.effective_user
            user_id = user.id
            ...
            if not update.message.photo:
                # User might have sent text instead of photo
                if update.message.text:
                    if update.message.text == "🏠 Main Menu":
                        await self.back_to_menu_keyboard(update, context)
            ...
            if amount <= 0:
                await update.message.reply_text("❌ Error: No amount specified. Please start over.")
                return ConversationHandler.END
            ...
            if Config.ADMIN_IDS and len(Config.ADMIN_IDS) > 0:
                    ...
                    try:
                        ...
                        try:
                            await self.application.bot.send_message(
                                chat_id=admin_id,
                                text=f"📥 Payment #{payment_id} received from {user_info} ({user_id}) for {amount} THB but failed to send screenshot. Error: {e}"
                            )
                        except Exception as e2:
                            logger.error(f"Also failed to send text to admin {admin_id}: {e2}")
            else:
                logger.error("No admin IDs configured! Payment will not be approved.")
                await update.message.reply_text("⚠️ Warning: No admin IDs configured. Please contact administrator.")
    ...
    async def create_account_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Start account creation process - FIXED"""
        query = update.callback_query
        await query.answer()
        ...
        await query.edit_message_text(
            ...
            "Note: This username will be used for VPN authentication."
        )
        
        return CREATE_USERNAME
    
    async def get_username(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Get VPN username from user - FIXED"""
        ...
        await update.message.reply_text(
            ...
            "Note: This password will be used for VPN authentication."
        )
        
        return CREATE_PASSWORD
    
    async def get_password(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Get VPN password from user - FIXED"""
        ...
        await update.message.reply_text(
            f"✅ Password accepted!\n\n"
            f"Username: {context.user_data['vpn_username']}\n"
            f"Password: {context.user_data['vpn_password']}\n\n"
            f"Now select subscription plan:",
            reply_markup=reply_markup
        )
    ...
    async def select_plan(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not username or not password:
            await query.edit_message_text("❌ Error: Missing username or password. Please start over.")
            return ConversationHandler.END
...
"""
    ...
    async def renew_account_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Clear any previous data
        context.user_data.clear()
        context.user_data['expecting_renew_password'] = True
    ...
    async def get_renew_password(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        context.user_data['expecting_renew_password'] = False
        ...
        await update.message.reply_text(
            f"✅ Account found!\n"
            f"Username: {username}\n"
            f"Current expiry: {expire_date}\n\n"
            f"Select renewal plan:",
            reply_markup=reply_markup
        )
    ...
    async def select_renew_plan(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not account_id:
            await query.edit_message_text("❌ Error: Account ID not found. Please start over.")
            return ConversationHandler.END
    ...
    async def handle_admin_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data.startswith('action_approve_'):
            ...
            if success:
                ...
                new_text = f"✅ Payment #{payment_id} approved!\nAmount: {amount} {Config.CURRENCY}\nUser ID: {user_id}\nApproved by: @{admin_name}"
...
EOF
    ...
    if python3 -c "import json; json.load(open('/etc/zivpn/config.json'))" 2>/dev/null; then
        ...
        if [ ${#VPN_PASSWORDS[@]} -gt 0 ]; then
            ...
            print_status "No passwords in config (users will add their own)"
        fi
    else
...
test_vpn_connection() {
    ...
    # Test with first password if exists
    if [ ${#VPN_PASSWORDS[@]} -gt 0 ]; then
        ...
        print_status "No default passwords configured. Users will create their own passwords."
    fi
    ...
    if [ ${#VPN_PASSWORDS[@]} -gt 0 ]; then
        ...
        echo "Passwords: No default passwords (users create their own)"
    fi
    echo ""
...
show_installation_summary() {
    ...
    if [ ${#VPN_PASSWORDS[@]} -gt 0 ]; then
        ...
        echo -e "VPN Passwords: ${YELLOW}No default (users create their own)${NC}"
    fi
    ...
    echo -e "💳 Top-up Credit with screenshot to admin ✓ FIXED"
    echo -e "✅❌ Admin Approve/Reject buttons ✓ FIXED"
    echo -e "🆕 Create Account with auto VPN config update ✓ FIXED"
    ...
    if [ ${#VPN_PASSWORDS[@]} -gt 0 ]; then
        ...
        echo -e "Password: (Create account via bot to get password)"
    fi
    ...
    if [ ${#VPN_PASSWORDS[@]} -gt 0 ]; then
        ...
        echo -e "   Password: (Create via bot first)"
    fi
    ...
    echo -e "\n${RED}=== FIXES APPLIED ===${NC}"
    echo -e "1. ✅ Payment notification to admin - FIXED"
    echo -e "2. ✅ Create account conversation flow - FIXED"
    echo -e "3. ✅ Default 'zi' password issue - REMOVED"
    echo -e "4. ✅ Users create their own passwords"
}
