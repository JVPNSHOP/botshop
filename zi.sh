#!/usr/bin/env bash
# install_zivpn_bot_final_fixed.sh
# Fixed version: fixes for duration buttons, menu renew, user account list/manage
# Run as root: sudo bash install_zivpn_bot_final_fixed.sh
set -euo pipefail
...
cat > "$BOT_PY" <<'PY'
#!/usr/bin/env python3
# bot.py - ZIVPN Telegram Bot (final fixed)
...
async def create_duration_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Format success message with icons
    success_msg = (
...
async def user_account_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("user_view_"):
        ...
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🔄 Renew", callback_data=f"user_renew_{a['id']}"),
             InlineKeyboardButton("🗑 Delete", callback_data=f"user_delete_{a['id']}")],
            [InlineKeyboardButton("🔙 Back to My Accounts", callback_data="my_accounts")]
        ])
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(user_account_cb, pattern=r"^(user_view_|user_delete_|user_renew_|my_accounts)"))
    ...
    app.add_handler(CallbackQueryHandler(create_duration_cb, pattern=r"^(dur30|dur60|dur90|cancel_create)$"))
...
[Unit]
Description=ZIVPN Telegram Bot (final fixed)
After=network.target
...
echo "⚙️ Use: systemctl [status|restart|stop|start] zivpn-bot.service"
echo ""
echo "📝 Features:"
echo "  ✅ Fixed create account duration selection"
echo "  ✅ Improved success message with icons"
echo "  ✅ Renew/Delete buttons after account creation"
