...
cat > "$BOT_PY" <<'PY'
#!/usr/bin/env python3
# ZIVPN Telegram Bot (with Add Account confirmation)
...
async def create_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Update ZIVPN config: auth.config list
    try:
...
async def admin_cb_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_list_users":
        ...
        for i, u in enumerate(users, 1):
            lines.append(
                f"{i}. 👤 ID: `{u['id']}` | 💰 Credit: `{u.get('credit', 0)}` Ks | "
        ...
        await query.edit_message_text("📁 *Account List:*\n\n" + "\n".join(lines),
                                      parse_mode="Markdown")
