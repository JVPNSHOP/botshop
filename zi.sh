#!/usr/bin/env bash
# install_zivpn_bot_full.sh
# Full installer: ZIVPN server + Telegram bot (complete bot.py embedded)
# Run as root: sudo bash install_zivpn_bot_full.sh
set -euo pipefail
...
if curl -fsSL "$ZIVPN_RELEASE_URL" -o "$ZIVPN_BIN"; then
  ...
  echo "Warning: failed to download zivpn binary (continuing if binary exists)"
fi

# --- Ensure config.json listens on all interfaces ---
...
# Tuning sysctl for forwarding + socket buffers
...
# iptables NAT initial placeholder (will reconfigure after prompt)
if [ -n "$SERVER_IP" ]; then
...
read -rp "Enter Your Server IP Address (public IP): " INPUT_SERVER_IP
...
read -rp "Enter QR Code Link (optional, press Enter to skip): " QR_LINK

# re-apply iptables NAT now that SERVER_IP is set
if [ -n "$SERVER_IP" ]; then
  # remove any previous rule that matches default placeholder (best-effort)
...
# save simple iptables persistent rules if netfilter-persistent exists
...
cat > "$BOT_PY" <<'PY'
#!/usr/bin/env python3
# bot.py - ZIVPN Telegram Bot (fixed with menu return)
...
# Duration callback for creation
async def create_duration_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Format success message with icons - EXACTLY as requested
    success_msg = (
    ...
    # Create inline keyboard with renew and delete buttons
    ...
    # Send main menu after account creation
...
async def my_accounts_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Also send each account with buttons
...
async def user_account_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # User renew start
        ...
        # Build durations based on credit
        kb = []
        if credit >= 50:
...
async def user_renew_choose_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Parse data: user_renew_choose_{account_id}_{days}
    parts = data.split("_")
    if len(parts) != 5:
...
async def admin_cb_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_list_users":
        ...
        # Count active accounts (not expired)
        now = time.time()
...
async def text_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt in ["💳 Top-up Credit", "top-up", "topup", "Top-up"]:
        ...
        return await my_accounts_cmd(update, context)  # Opens account list
...
apt-get install -y python3-venv python3-pip curl jq iptables-persistent ufw 1>/dev/null 2>/dev/null || true
...
echo "  - Check server public IP is correct: echo $SERVER_IP"
