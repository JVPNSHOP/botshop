...
# Full ZIVPN + Telegram bot installer + Auto-Expire monitor
...
# - Auto-Expire monitor: watches server journal for "client connected" events,
#   if same password (id) is seen from >1 distinct source -> expire & remove password.
...
############################
# BOT PYTHON CODE
# (includes SIM PACKAGE, Admin Notify, etc.)
############################

cat > "$BOT_PY" <<'PY'
#!/usr/bin/env python3
# ZIVPN Telegram Bot (with Add Account confirmation + admin custom date + admin unlimited + SIM info + admin notify)

import json, logging, os, time, datetime, uuid, socket, subprocess, traceback
...
# Admin notify states stored in context.user_data["admin_notify_state"] when active:
# "awaiting_target" -> next message should be "all" or user id
# "awaiting_message" -> next message is the message to send
# Conversation functions below handle that flow.
...
def account_status_info(expiry_ts):
    """
    Returns (status_text, days_left)
    Status:
      🟢 Online     -> expiry > 5 days from now
      🟠 5 Days Left -> expiry within next 5 days (and not expired)
      🔴 Offline    -> expired
    """
    try:
        now = int(time.time())
...
async def bind_device_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid = update.effective_user.id
    accounts = read_json(ACCOUNTS, [])
    # find the account owned by this user (existing behavior)
    ...
    # If different device tries to bind (second device) -> record and auto-expire
    # Append to used_devices
    used.append({"device_id": device_id, "bound_at": int(time.time())})
    acc["used_devices"] = used

    # Auto-expire: set expiry to now and remove password from ZIVPN so it can't be used
    try:
        now_ts = int(time.time())
        acc["expiry"] = now_ts
        write_json(ACCOUNTS, accounts)
        # remove the password from zivpn auth list
        remove_password_from_zivpn(acc.get("password", ""))
    except Exception as e:
        log.error("Error while auto-expiring account on multi-device bind: %s", e)

    # Notify the user and the admin
    await update.message.reply_text(
        "❌ Detected binding from a second device. For security the account has been expired automatically. "
        "Please contact admin if you need to restore access.",
        reply_markup=main_menu_kb()
    )
    try:
        await context.bot.send_message(
            chat_id=ADMIN_ID,
            text=(
                f"⚠️ Auto-expire: Account `{acc.get('username')}` (owner {uid}) was bound from an additional device (`{device_id}`).\n"
                "The account has been expired and password removed from ZIVPN."
            )
        )
    except Exception:
        pass
...
############################
# MONITOR PYTHON (auto-expire + owner/admin notify + GRACE_WINDOW)
############################
...
auto-expire the account(s) with that password:
 - set expiry to now in /etc/zivpn/accounts.json
...
"""
import subprocess, json, re, time, os, sys, urllib.request, urllib.parse, traceback
...
# GRACE_WINDOW: seconds window to consider multi-source as sharing (adjust to reduce false positives)
...
expired = set()  # passwords already expired (avoid repeated actions)
...
def write_json_atomic(path, obj):
    tmp = "/tmp/tmp_zivpn_" + os.path.basename(path)
    with open(tmp, "w") as f:
        json.dump(obj, f, indent=2, ensure_ascii=False)
    os.replace(tmp, path)

def remove_password_from_config(password):
    try:
        cfg = read_json(ZIVPN_CONFIG, {})
        auth = cfg.get("auth", {})
        cfg_list = auth.get("config", [])
        if password in cfg_list:
            cfg_list.remove(password)
            auth["config"] = cfg_list
            cfg["auth"] = auth
            write_json_atomic(ZIVPN_CONFIG, cfg)
            # restart service
            subprocess.run(["systemctl", "restart", "zivpn.service"], check=False)
            return True
    except Exception as e:
        print("remove_password error:", e)
    return False

def expire_accounts_by_password(password):
    ...
    if changed:
        ...
        removed = remove_password_from_config(password)
...
def process_line(line):
    ...
    try:
        ...
        print(f"[monitor] pid={pid} ips={unique_ips}")
        if len(unique_ips) > 1 and pid not in expired:
            print(f"[monitor] multi-source detected for pid={pid} -> expiring")
            try:
                removed, owners = expire_accounts_by_password(pid)
                expired.add(pid)
                # notify admin
                cfg = read_json(BOT_CONFIG, {})
                admin = cfg.get("admin_id")
                admin_msg = f"⚠️ Auto-expire: password `{pid}` used from multiple IPs {list(unique_ips)}. Accounts expired and password removed from config. Removed from config: {removed}"
                if admin:
                    notify_telegram(admin, admin_msg)
                # notify owners
                owner_msg = f"⚠️ သတင်း — သင့်အကောင့် (password `{pid}`) သည် တခြား device မှလည်း ချိတ်ဆက်လိုက်သောကြောင့် အလိုအလျှောက် expired လုပ်ထားလိုက်သည်။\n\nPassword ကို share မလုပ်ရန် အကြံပြုပါသည်။"
                for o in owners:
                    try:
                        if o:
                ...
                print("expire error:", e, traceback.format_exc())
    except Exception as e:
        print("parse error", e)
...
[Unit]
Description=ZIVPN Auth Monitor (auto-expire)
