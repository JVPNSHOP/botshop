#!/usr/bin/env bash
# install_zivpn_full_fix_with_confirm_create.sh
# Full ZIVPN + Telegram bot installer with Add Account confirmation button
# Run as root: sudo bash install_zivpn_full_fix_with_confirm_create.sh
...
JSON
  ...
  # ensure cert/key/auth/config (using jq if available)
  if command -v jq >/dev/null 2>&1; then
    jq '.cert = "/etc/zivpn/zivpn.crt" | .key = "/etc/zivpn/zivpn.key" | .auth = (.auth // {"Mode":"password"}) | .config = (.config // ["zi"])' "$ZIVPN_ETC/config.json" > "$ZIVPN_ETC/config.json.tmp" 2>/dev/null && mv "$ZIVPN_ETC/config.json.tmp" "$ZIVPN_ETC/config.json" || true
  else
    # best effort text inserts if jq not present
    if ! grep -q '"cert"' "$ZIVPN_ETC/config.json"; then
      sed -i '1s|{|{"cert":"/etc/zivpn/zivpn.crt",|' "$ZIVPN_ETC/config.json" || true
    fi
    if ! grep -q '"key"' "$ZIVPN_ETC/config.json"; then
      sed -i '1s|{|{"key":"/etc/zivpn/zivpn.key",|' "$ZIVPN_ETC/config.json" || true
    fi
    if ! grep -q '"auth"' "$ZIVPN_ETC/config.json"; then
      sed -i '1s|{|{"auth":{"Mode":"password"},|' "$ZIVPN_ETC/config.json" || true
    fi
    if ! grep -q '"config"' "$ZIVPN_ETC/config.json"; then
      sed -i '1s|{|{"config":["zi"],|' "$ZIVPN_ETC/config.json" || true
    fi
  fi
fi
...
cat > "$BOT_PY" <<'PY'
#!/usr/bin/env python3
# bot.py - ZIVPN Telegram Bot (embedded) with Add Confirm flow
...
# Conversation states
(CREATE_USERNAME, CREATE_PASSWORD, CREATE_DAYS) = range(3)
...
# Admin approve/deny/unlimited (unchanged)
...
# Create account conversation (modified)
...
async def create_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ask for expiry days (explicit)
    await update.message.reply_text("Send number of days for expiry (30 / 60 / 90):")
    return CREATE_DAYS

async def create_days(update: Update, context: ContextTypes.DEFAULT_TYPE):
    txt = update.message.text.strip()
    # validate integer 30/60/90
    try:
        days = int(txt)
    except:
        await update.message.reply_text("Invalid number. Please send 30, 60 or 90 (just the number).")
        return CREATE_DAYS
    if days not in (30,60,90):
        await update.message.reply_text("Only 30, 60 or 90 days are allowed. Send 30, 60 or 90.")
        return CREATE_DAYS

    # store days and show confirmation with Add / Cancel
    ...
    confirm_text = (
        ...
        f"💰 Cost: `{cost}` Ks\n"
        f"📅 Expiry: `{expiry_str}`\n\n"
        "Press *Add Account ✅* to create (will deduct credit), or *Cancel ❌* to abort."
    )

    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("Add Account ✅", callback_data="confirm_add") , InlineKeyboardButton("Cancel ❌", callback_data="confirm_cancel")]
    ])

    await update.message.reply_text(confirm_text, parse_mode="Markdown", reply_markup=kb)
    return ConversationHandler.END  # We'll handle final action via callback handler

# Callback when pressing Add Account or Cancel
async def confirm_add_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if query.from_user.id is None:
        await query.edit_message_text("Invalid user.")
        return

    uid = query.from_user.id
    # retrieve pending data from user's last messages context - using persistent storage in files is safer,
    # but for simplicity we read from context.user_data (it exists if the same bot instance handles it).
    # NOTE: If context.user_data was lost (bot restart), we must notify user.
    ud = context.user_data
    uname = ud.get("create_username")
    upass = ud.get("create_password")
    days = ud.get("create_days")

    if not (uname and upass and days):
        await query.edit_message_text("❌ Could not find pending account data (bot restarted or session expired). Please /create again.")
        return
    ...
    if credit < cost:
        await query.edit_message_text("❌ Insufficient credit to create account. Top-up first.", reply_markup=main_menu_kb())
        return
    ...
    if u:
        ...
        users.append({"id": uid, "username": "", "credit": 0})
        # Should not reach here since credit insufficient already checked, but keep safe.
    write_json(USERS, users)
    ...
    # Add password to zivpn config (update config.json)
    try:
        zcfg = read_json(ZIVPN_CONFIG, {})
        # try both possible keys: "config" or nested under auth, handle both
        if "config" not in zcfg:
            zcfg["config"] = ["zi"]
        if upass not in zcfg["config"]:
            zcfg["config"].append(upass)
        # also ensure auth mode is set correctly if needed
        if "auth" not in zcfg:
            zcfg["auth"] = {"Mode": "password"}
    ...
    # notify admin and clear temp data
    await context.bot.send_message(chat_id=ADMIN_ID, text=f"📝 New account created by {uid}\n👤 {uname}\n⏳ {days} days\n📅 Expires: {expiry_str}")
    # clear stored temporary fields
    for k in ("create_username","create_password","create_days"):
        if k in context.user_data:
            del context.user_data[k]

async def confirm_cancel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    for k in ("create_username","create_password","create_days"):
        if k in context.user_data:
            del context.user_data[k]
    await query.edit_message_text("❌ Account creation cancelled.", reply_markup=None)
    # send main menu
    await context.bot.send_message(chat_id=query.from_user.id, text="Cancelled. Choose an option:", reply_markup=main_menu_kb())

# ... rest of bot handlers (bind, my_accounts, renew, admin) unchanged ...
# For brevity we keep the previous implementations (they were included earlier in original script).
# We'll re-add admin panel, account list, renew handlers, etc., same as before.

# (Due to space we include the previously implemented handlers exactly as in the original script.)
# Insert the rest of handlers here (bind_device_cmd, my_accounts_cmd, user_account_cb, user_renew_choose_cb, admin_panel, admin_cb_router, text_menu_handler, cancel_conversation)
# For correctness, we will add them below (copied from your script):
...
# Admin panel and router (same as earlier)...
@admin_only
...
def main():
    ...
    # Add confirm Add/Cancel callbacks
    app.add_handler(CallbackQueryHandler(confirm_add_cb, pattern=r"^confirm_add$"))
    app.add_handler(CallbackQueryHandler(confirm_cancel_cb, pattern=r"^confirm_cancel$"))
...
[Unit]
Description=ZIVPN Telegram Bot (fixed with Add Confirm)
After=network.target
