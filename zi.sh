#!/usr/bin/env bash
# install_zivpn_bot_fixed2.sh
# ZIVPN UDP Installer + Telegram Bot (fixed: duration callback, QR image, admin unlimited/approve)
# Run as root: sudo bash install_zivpn_bot_fixed2.sh
set -euo pipefail
...
cat > "$BOT_PY" <<'PY'
#!/usr/bin/env python3
# ZIVPN Telegram Bot - fixed v2 (duration routing + admin unlimited + icons + QR image)
...
async def topup_button_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if QR_LINK:
        try:
            ...
            # ignore if QR_LINK not a direct image
            pass
...
# Admin callbacks: approve/deny/unlimited
...
def main():
    ...
    # callback handlers (register both create-duration and global patterns)
    ...
    app.add_handler(CallbackQueryHandler(create_duration_cb, pattern="^(dur30|dur60|dur90|cancel_create)$"))  # global backup
...
[Unit]
Description=ZIVPN Telegram Bot (fixed2)
After=network.target
