...
############################
# BOT PYTHON CODE
# (unchanged from prior but kept here in full; includes account status display)
############################
...
When same 'id' (password/username) is seen from >1 distinct addr,
auto-expire the account(s) with that password:
 ...
 - notify admin via Telegram bot (if configured)
"""
...
# track seen sources per id (password)
seen = {}  # id -> set(addr)
expired = set()  # passwords already expired (avoid repeated actions)
# tolerance: once >1 distinct addr -> expire immediately
...
def notify_admin(text):
    ...
    if not token or not admin:
        print("no bot token / admin configured; skip notify")
        return
    try:
        data = {
            "chat_id": admin,
...
def process_line(line):
    ...
    try:
        ...
        obj = json.loads(js)
        addr = obj.get("addr")
        pid = obj.get("id")  # id = the username/password field in server log
        ...
        s.add(addr)
        print(f"[monitor] seen pid={pid} addr={addr} count={len(s)}")
        if len(s) > 1 and pid not in expired:
            print(f"[monitor] multi-source detected for pid={pid} -> expiring")
            try:
                removed = expire_accounts_by_password(pid)
                expired.add(pid)
                notify_admin(f"⚠️ Auto-expire: password `{pid}` was used from multiple sources {list(s)}. Accounts expired and password removed from config. Removed from config: {removed}")
            except Exception as e:
