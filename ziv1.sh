...
# - ZIVPN config uses: auth.mode="passwords", auth.config=["zi",...]
# - Broad UDP DNAT so client can enter ONLY IP (no :port) in ZIVPN app
...
# ALWAYS write a clean working config.json
...
JSON
echo "Wrote fresh /etc/zivpn/config.json"
...
# clear PREROUTING (careful: wipes previous DNAT rules)
iptables -t nat -F PREROUTING 2>/dev/null || true

# recommended range: 6000-19999 -> 5667
iptables -t nat -A PREROUTING -i "$INET_IF" -p udp --dport 6000:19999 \
  -j DNAT --to-destination "$SERVER_IP":5667 1>/dev/null 2>/dev/null || true

# optional BROAD DNAT so client using ONLY IP (no :port) also works
if [ "$BROAD_DNAT" = true ]; then
  echo "Adding BROAD DNAT: udp ports 1:65535 -> ${SERVER_IP}:5667"
  iptables -t nat -C PREROUTING -i "$INET_IF" -p udp --dport 1:65535 \
    -j DNAT --to-destination "$SERVER_IP":5667 2>/dev/null || \
  iptables -t nat -A PREROUTING -i "$INET_IF" -p udp --dport 1:65535 \
    -j DNAT --to-destination "$SERVER_IP":5667 1>/dev/null 2>/dev/null || true
fi
...
cat > "$BOT_PY" <<'PY'
#!/usr/bin/env python3
# bot.py - ZIVPN Telegram Bot (with Add Account confirm + ZIVPN auth.passwords update)
...
########################
# Helpers for ZIVPN config
########################

def add_password_to_zivpn(passwd: str):
    """Ensure passwd is added to ZIVPN auth.config list (mode=passwords)."""
    try:
        zcfg = read_json(ZIVPN_CONFIG, {})
        auth = zcfg.get("auth") or {}
        # normalise key name
        mode = auth.get("mode") or auth.get("Mode")
        auth["mode"] = "passwords"
        pw_list = auth.get("config")
        if not isinstance(pw_list, list):
            pw_list = []
        if passwd not in pw_list:
            pw_list.append(passwd)
        auth["config"] = pw_list
        zcfg["auth"] = auth
...
def remove_password_from_zivpn(passwd: str):
    try:
        zcfg = read_json(ZIVPN_CONFIG, {})
        auth = zcfg.get("auth") or {}
        pw_list = auth.get("config")
        if isinstance(pw_list, list) and passwd in pw_list:
            pw_list.remove(passwd)
            auth["config"] = pw_list
            zcfg["auth"] = auth
            write_json(ZIVPN_CONFIG, zcfg)
    except Exception as e:
        log.error("Failed cleanup in ZIVPN config: %s", e)
...
async def admin_topup_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("unlimited_"):
        ...
        await context.bot.send_message(
            chat_id=uid,
            text="♾ Admin granted you unlimited credit.",
            reply_markup=main_menu_kb()
        )
        await query.edit_message_caption(
...
########################
# Create account (with Add Account ✅ / Cancel ❌)
########################
...
async def create_duration_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """User taps 30/60/90 days -> show confirmation with Add Account / Cancel."""
    query = update.callback_query
    await query.answer()
    data = query.data
    uid = query.from_user.id

    if data not in ("dur30", "dur60", "dur90"):
    ...
    # store preview info in user_data
    context.user_data["preview_days"] = days
    context.user_data["preview_cost"] = cost
    context.user_data["preview_expiry_ts"] = expiry_ts
    ...
    text = (
        ...
        f"⏳ *Days:* `{days}`\n"
        f"💰 *Cost:* `{cost}` Ks\n\n"
        "➡️ Press *✅ Add Account* to create, or *❌ Cancel* to abort."
    )

    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Add Account", callback_data="confirm_create"),
         InlineKeyboardButton("❌ Cancel", callback_data="cancel_create")]
    ])

    await query.edit_message_text(text, parse_mode="Markdown", reply_markup=kb)
    # still in CHOOSE_DURATION until user clicks Add/Cancel
    return CHOOSE_DURATION

async def create_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle Add Account ✅ / Cancel ❌ buttons."""
    query = update.callback_query
    await query.answer()
    data = query.data
    uid = query.from_user.id
    ...
    uname = context.user_data.get("create_username")
    upass = context.user_data.get("create_password")
    days = context.user_data.get("preview_days")
    cost = context.user_data.get("preview_cost")
    expiry_ts = context.user_data.get("preview_expiry_ts")

    if not all([uname, upass, days, cost, expiry_ts]):
        await query.edit_message_text("❌ Session data missing. Please try /create again.", reply_markup=main_menu_kb())
        context.user_data.clear()
        return ConversationHandler.END
    ...
    # update zivpn auth.config
    add_password_to_zivpn(upass)

    # IP detection for message
    server_ip = SERVER_IP
    ...
    success_msg = (
        ...
        f"📅 *Expired Date:* `{expiry_str}`\n"
        f"⏳ *Days:* `{days}`\n\n"
        "💡 *Note:* This account is restricted to 1 device. "
        "Use /bind <device-id> from your first device to bind it."
    )
...
async def user_account_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "my_accounts_list":
        # reuse my_accounts listing
        fake_update = Update(update.update_id, message=None)
        fake_update._effective_user = query.from_user  # type: ignore
        await my_accounts_cmd(update._bot._Application__update_queue, context)  # not ideal; ignore
        # simpler: just tell user to use menu
        await query.edit_message_text("Please tap 🗂 My Accounts from menu.", reply_markup=main_menu_kb())

########################
# Renew choose handler
########################
...
########################
# Topup menu callback (from buttons)
########################
...
async def admin_cb_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_list_users":
        ...
        for i, u in enumerate(users, 1):
            lines.append(
                f"{i}. 👤 ID: `{u['id']}` | 💰 Credit: `{u.get('credit', 0)}` Ks | 🏷️ Username: @{u.get('username', 'N/A')}"
            )
        text = "👥 *User List:*\n\n" + "\n".join(lines)
        ...
        for i, a in enumerate(accounts, 1):
            ...
            lines.append(
                f"{i}. 👤 `{a['username']}` | 👤 Owner: `{a['created_by']}` | "
                f"📅 Exp: {exp} | 📱 Bound: `{a.get('bound_device_id', 'None')}`"
            )
        text = "📁 *Account List:*\n\n" + "\n".join(lines)
        await query.edit_message_text(text, parse_mode="Markdown")
        ...
        for t in pending:
            ...
            if t.get("photo_file_id"):
                await context.bot.send_photo(
                    chat_id=ADMIN_ID, photo=t['photo_file_id'], caption=text,
...
def main():
    ...
    # Conversation handler for create account with confirmation
    conv = ConversationHandler(
        ...
        states={
            ...
            CHOOSE_DURATION: [
                CallbackQueryHandler(create_duration_cb, pattern=r"^(dur30|dur60|dur90)$"),
                CallbackQueryHandler(create_confirm_cb, pattern=r"^(confirm_create|cancel_create)$"),
            ],
        },
...
apt-get install -y python3-venv python3-pip curl jq iptables-persistent ufw tcpdump \
